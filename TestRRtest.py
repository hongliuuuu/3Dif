...
def RR_rf_dis(n_trees, X,Y,train_indices,test_indices,seed):
    ...
    clff = clf.fit(X[train_indices], Y[train_indices])
    pred = clff.predict_proba(X[test_indices])
...
def mcode(ite):
    ...
    for ii in range(1):
        ...
        seed = 1000+ii
        train_indices, test_indices = splitdata(X=X[:10, :], Y=Y, ratio=R, seed=seed)
        print("Start rest")
        # view1

        X_features_train1, X_features_test1, w1, pred1 = RR_rf_dis(n_trees=10, X=Xnew1, Y=Y, train_indices=train_indices,
...
if __name__ == '__main__':
    Parallel(n_jobs=1)(delayed(mcode)(ite=i) for i in range(1))